...
- **Tell, don't ask.** `account.Deposit(amount)` beats `if (account.Balance + amount < limit) account.Balance += amount`. Logic that reads private state belongs _on_ the type that owns it.
