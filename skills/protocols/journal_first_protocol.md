...
keywords: [journal, prior, history, lesson, recall, previous, before, start, task, lookup, check, notes, memory]
token_cost: 140
...
4. If a relevant entry exists, treat its *Approach that worked* as the default plan and its *What I tried first and why it failed* as the things to avoid. Verify it still applies (file paths, dependency versions) before blindly following.
